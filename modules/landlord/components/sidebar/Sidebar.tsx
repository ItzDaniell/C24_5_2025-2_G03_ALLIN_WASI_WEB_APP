    ...
    >
      <div className="p-4 border-b border-white/10 relative">
        ...
        {variant === "desktop" && onToggle && (
          <button
            onClick={onToggle}
            className="absolute -right-3 cursor-pointer top-1/2 -translate-y-1/2 w-6 h-6 bg-creme-brulee rounded-full flex items-center justify-center shadow-md hover:scale-110 transition-all z-10"
        ...
        {menuItems.map((item) => {
          ...
          return (
            <Button
              key={item.id}
              variant="ghost"
              className={`w-full h-11 cursor-pointer transition-all duration-300 relative group ${expanded ? 'justify-start px-4' : 'justify-center'} ${isActive ? "bg-creme-brulee text-white shadow-md hover:bg-creme-brulee hover:text-white" : "text-au-lait/70 hover:bg-white/5 hover:text-white"}`}
