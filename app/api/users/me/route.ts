...
export async function GET() {
  ...
  const response = await serverFetch(`${BASE_URL}/users/me`, { method: "GET" });
  ...
  if (!response.ok) {
    return Response.json(
      { error: (data as any)?.message || "[NextAPI]: Error al obtener el perfil" },
...
export async function PUT(request: Request) {
  ...
  const response = await serverFetch(`${BASE_URL}/users/me`, {
    method: "PUT",
    headers: {
